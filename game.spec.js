...
describe('Life game', function() {
    describe('model', function() {
        ...
        it('should return dead state when live cell has no live neighbours', function() {
            var cell = {state: true, neighbours: []};
            expect(model.newStateForCell(cell)).toBe(false);
        });

        it('should return dead state when live cell has one live neighbour', function() {
            var cell = {state: true, neighbours: [true]};
            expect(model.newStateForCell(cell)).toBe(false);
        });
        
        it('should return live state when live cell has two live neighbours', function() {
            var cell = {state: true, neighbours: [true, true]};
            expect(model.newStateForCell(cell)).toBe(true);
        });
        
        it('should return live state when live cell has three live neighbours', function() {
            var cell = {state: true, neighbours: [true, true, true]};
            expect(model.newStateForCell(cell)).toBe(true);
        });
        
        it('should return dead state when live cell has four live neighbours', function() {
            var cell = {state: true, neighbours: [true, true, true, true]};
            expect(model.newStateForCell(cell)).toBe(false);
        });

        it('should return live state when live cell has four dead and three live neighbours', function()               {
            var cell = {state: true, neighbours: [false, true, false, true, false, true, false]};
            expect(model.newStateForCell(cell)).toBe(true);
        });

        it('should return dead state when live cell has eight live neighbours', function() {
            var cell = {state: true, neighbours: [true, true, true, true, true, true, true, true]};
            expect(model.newStateForCell(cell)).toBe(false);
        });
        
        it('should return dead state when live cell has eight dead neighbours', function() {
            var cell = {state: true, neighbours: [false, false, false, false, false, false, false, false]};
            expect(model.newStateForCell(cell)).toBe(false);
        });
        
        it('should return dead state when dead cell has no live neighbours', function() {
            var cell = {state: false, neighbours: []};
            expect(model.newStateForCell(cell)).toBe(false);
        });

        it('should return dead state when dead cell has one live neighbour', function() {
            var cell = {state: false, neighbours: [true]};
            expect(model.newStateForCell(cell)).toBe(false);
        });
        
        it('should return dead state when dead cell has two live neighbours', function() {
            var cell = {state: false, neighbours: [true, true]};
            expect(model.newStateForCell(cell)).toBe(false);
        });
        
        it('should return live state when dead cell has three live neighbours', function() {
            var cell = {state: false, neighbours: [true, true, true]};
            expect(model.newStateForCell(cell)).toBe(true);
        });
        
        it('should return same pattern when given a block still-life', function() {
            var inputGrid = [
        ...
        it('should return next pattern when given a blinker oscillator', function() {
            var inputGrid = [
        ...
        it('should return to original pattern when oscillating a blinker twice', function() {
            var inputGrid = [
               [false, false, false, false, false],
               [false, false, false, false, false],
               [false, true, true, true, false],
